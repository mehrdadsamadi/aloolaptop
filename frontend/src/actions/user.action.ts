...
export async function changeUserRole(userId: string, role: Roles) {
  const res = await apiFetch(ENDPOINTS.USER.CHANGE_ROLE(userId), { method: 'PATCH', body: JSON.stringify({ role }) })
